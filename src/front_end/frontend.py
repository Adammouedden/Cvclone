import textwrap

def generate_cvclone_page_with_chat_logic():
    """Generates the HTML, CSS, and JavaScript content for the Cvclone page 
    with chat visible on Home and Chat tabs only."""

    # Note: The provided API Key "AIzaSyBQQhjLjjYo4CCV3yil5JBCxpoBOz7qExY" is used as requested.

    html_content = textwrap.dedent(r"""
    <!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Cvclone</title>
    <style>
        body {
            font-family: Arial, sans-serif;
            margin: 0;
            display: flex;
            height: 100vh;
            overflow: hidden; /* Prevent body scroll */
            background-color: #f4f4f9;
        }

        /* --- Sidebar/Navigation --- */
        .sidebar {
            width: 200px;
            background-color: #2c3e50; /* Dark sidebar */
            color: white;
            display: flex;
            flex-direction: column;
            padding: 15px 0;
            box-shadow: 2px 0 5px rgba(0,0,0,0.1);
        }

        .sidebar-title {
            text-align: center;
            font-size: 1.5em;
            font-weight: bold;
            margin-bottom: 20px;
            padding: 0 10px;
            color: #ecf0f1;
        }

        .nav-link {
            padding: 15px 20px;
            cursor: pointer;
            text-decoration: none;
            color: #bdc3c7;
            border-left: 5px solid transparent;
            transition: all 0.3s ease;
        }

        .nav-link:hover {
            background-color: #34495e;
        }

        .nav-link.active {
            background-color: #34495e;
            color: white;
            border-left-color: #3498db; /* Highlight active tab */
        }

        /* --- Main Content Area --- */
        .content-area {
            flex-grow: 1;
            display: flex;
            flex-direction: column;
            background-color: white;
        }

        /* --- Tab Content --- */
        .tab-content {
            flex-grow: 1;
            padding: 20px;
            overflow-y: auto; /* Allow content scroll if needed */
            display: none;
        }

        .tab-content.active {
            display: block;
        }

        /* --- Home Tab (Map) --- */
        #map {
            height: 100%;
            width: 100%;
            min-height: 400px; /* Ensure map has space */
        }

        #home-content {
            height: 100%; /* Important for map to fill space */
            padding: 0;
        }

        /* --- Chat Tab (Chatbot UI) --- */
        #chat-content {
            display: flex;
            flex-direction: column;
            height: 100%;
            padding: 0;
        }

        .chat-window {
            flex-grow: 1;
            overflow-y: auto;
            padding: 15px;
            border: 1px solid #ddd;
            background-color: #f9f9f9;
            margin-bottom: 10px;
            border-radius: 5px;
        }

        .chat-input-container {
            display: flex;
            padding: 10px;
            border-top: 1px solid #eee;
        }

        #chat-input {
            flex-grow: 1;
            padding: 10px;
            border: 1px solid #ccc;
            border-radius: 4px;
            margin-right: 10px;
        }

        #send-btn {
            padding: 10px 15px;
            background-color: #9917bd;
            color: white;
            border: none;
            border-radius: 4px;
            cursor: pointer;
            transition: background-color 0.3s;
        }

        #send-btn:hover {
            background-color: #2980b9;
        }

        #civilian-send-btn {
            padding: 10px 15px;
            background-color: #9917bd;
            color: white;
            border: none;
            border-radius: 4px;
            cursor: pointer;
            transition: background-color 0.3s;
        }

        #enterprise-send-btn {
            padding: 10px 15px;
            background-color: #9917bd;
            color: white;
            border: none;
            border-radius: 4px;
            cursor: pointer;
            transition: background-color 0.3s;
        }

        #civilian-chat-input, #enterprise-chat-input {
            flex-grow: 1;
            padding: 10px;
            border: 1px solid #ccc;
            border-radius: 4px;
            margin-right: 10px;
        }

        .message-user, .message-bot {
            margin-bottom: 10px;
            padding: 8px 12px;
            border-radius: 15px;
            max-width: 70%;
        }

        .message-user {
            background-color: #d1e7dd; /* Light green-blue */
            align-self: flex-end;
            margin-left: auto;
        }

        .message-bot {
            background-color: #f0f0f0; /* Light gray */
            align-self: flex-start;
            margin-right: auto;
        }

        /* Chat panels are now separate tabs (civilian & enterprise) */

        .toggle-chat-btn {
            position: absolute;
            right: 60px;
            bottom: 20px;
            z-index: 60;
            padding: 14px 18px;
            border-radius: 999px;
            background-color: #9917bd; 
            color: white;
            border: none;
            box-shadow: 0 6px 16px rgba(52,152,219,0.28);
            cursor: pointer;
            font-size: 18px;    /* set text size */
            line-height: 1.2;
        }

        .locate-me-btn {
            position: fixed; /* fixed to viewport so it's always visible above the map */
            right: 100px;
            bottom: 25px;
            z-index: 9999; /* ensure it sits on top of map layers */
            padding: 10px 14px;
            border-radius: 6px;
            background-color: #2ecc71;
            color: white;
            border: none;
            box-shadow: 0 4px 12px rgba(46,204,113,0.18);
            cursor: pointer;
            font-size: 14px;
        }

        .coords-display {
            position: absolute;
            right: 20px;
            bottom: 76px;
            z-index: 65;
            background: rgba(0,0,0,0.7);
            color: #fff;
            padding: 8px 12px;
            border-radius: 6px;
            font-size: 13px;
            display: none;
        }

        /* About page layout */
        .about-container {
            display: flex;
            gap: 20px;
            align-items: flex-start;
        }

        .about-left {
            flex: 1 1 60%;
        }

        .about-right {
            flex: 0 0 240px;
            background: #fafafa;
            border-left: 1px solid #eee;
            padding-left: 16px;
        }

        .about-images {
            display: flex;
            gap: 12px;
            align-items: flex-start;
        }

        .about-images img.about-image {
            width: 50%; /* each image fills half the left column */
            height: auto;
            border-radius: 8px;
            display: block;
            margin-bottom: 12px;
        }
        /* Hurricanes folder UI */
        .hurricane-list {
            max-height: 70vh;
            overflow: auto;
            border: 1px solid #eee;
            padding: 12px;
            border-radius: 6px;
            background: #fafafa;
        }
        .storm-item { margin-bottom: 8px; }
        .storm-header { cursor: pointer; padding: 8px; background: #fff; border: 1px solid #e6e6e6; border-radius: 6px; }
        .storm-timestamps { margin-top: 6px; margin-left: 8px; display: none; }
        .storm-timestamps.active { display: block; }
        .timestamp-item { padding: 6px 8px; cursor: pointer; border-radius: 4px; }
        .timestamp-item:hover { background: #f0f8ff; }
    </style>
</head>
<body>

        <div class="sidebar">
        <div class="sidebar-title">Cvclone</div>
        <a class="nav-link active" data-tab="home" onclick="openTab('home')">Home</a>
        <a class="nav-link" data-tab="resources" onclick="openTab('resources')">Resources</a>
        <a class="nav-link" data-tab="civilian" onclick="openTab('civilian')">Civilian</a>
        <a class="nav-link" data-tab="enterprise" onclick="openTab('enterprise')">Enterprise</a>
        <a class="nav-link" data-tab="about" onclick="openTab('about')">About me</a>
        <a class="nav-link" data-tab="hurricanes" onclick="openTab('hurricanes')">Hurricanes</a>
    </div>

    <div class="content-area">

        <div id="home-content" class="tab-content active">
            <div id="map"></div>

            <!-- Locate button + coords display -->
            <button id="locate-me-btn" class="locate-me-btn">Use my location</button>
            <div id="coords-display" class="coords-display" aria-live="polite"></div>
        </div>

        <!-- Civilian chat tab -->
        <div id="civilian-content" class="tab-content">
            <h2>Civilian Chat</h2>
            <div class="chat-window" id="civilian-chat-window">
                <div class="message-bot">Civilian assistant ready.</div>
            </div>
            <div class="chat-input-container">
                <input type="text" id="civilian-chat-input" placeholder="Ask the civilian assistant...">
                <button id="civilian-send-btn">Send</button>
            </div>
        </div>

        <!-- Enterprise chat tab -->
        <div id="enterprise-content" class="tab-content">
            <h2>Enterprise Chat</h2>
            <div class="chat-window" id="enterprise-chat-window">
                <div class="message-bot">Enterprise assistant online.</div>
            </div>
            <div class="chat-input-container">
                <input type="text" id="enterprise-chat-input" placeholder="Ask the enterprise assistant...">
                <button id="enterprise-send-btn">Send</button>
            </div>
        </div>

        <!-- About Me tab -->
        <div id="about-content" class="tab-content">
            <h1>About me</h1>
            <div class="about-container">
                <div class="about-left">
                    <div class="about-images">
                        <img src="./thegang.jpg" alt="Gang photo" class="about-image">
                        <img src="./thegangfeelingsilly.jpg" alt="Silly gang" class="about-image">
                    </div>
                    <p>Hello — this project, Cvclone, is an experimental interface combining mapping, live overlays, and chat assistants. It demonstrates multi-agent coordination, realtime overlays, and a simple frontend to prototype ideas. This page is a short bio area where you can describe the project goals, background, and contact info.</p>
                </div>
                <div class="about-right">
                    <h2>Credits</h2>
                        <h4>Adam Mouedden</h4>
                        <h4>David Orjuela</h4>
                        <h4>Kenta Festag</h4>
                        <h4>Sofia Flores</h4>    
                </div>
            </div>
        </div>

        <div id="resources-content" class="tab-content">
            <h2>Resources</h2>
            <p>This tab is currently blank.</p>
        </div>

        <!-- Hurricanes tab -->
        <div id="hurricanes-content" class="tab-content">
            <h2>Hurricanes</h2>
            <p>Browse storms grouped by storm id. Expand a storm to see timestamps; click a timestamp to draw it on the map.</p>
            <div id="hurricane-list" class="hurricane-list">
                <p>Loading storms...</p>
            </div>
        </div>

        <!-- Chat tab removed; chat is available inside Home as a collapsible panel -->

    </div>

    <script>
        // --- Tab Switching Logic ---
        const tabContents = document.querySelectorAll('.tab-content');
        const navLinks = document.querySelectorAll('.nav-link');

    // --- Overlay configuration (edit these values to change the image and its bounds) ---
    // Change IMAGE_OVERLAY_URL to point to a different image file (relative to this HTML file)
    const IMAGE_OVERLAY_URL = './ultimateimage.png';

    // Two ways to configure overlay bounds:
    // 1) Explicit bounds (recommended if you already know them):
    //    { north: <lat>, south: <lat>, east: <lng>, west: <lng> }
    // 2) Center + dimensions in meters (easier to edit). Set
    //    IMAGE_OVERLAY_USE_CENTER = true and provide IMAGE_OVERLAY_CENTER + width/height.

    // Use explicit bounds by default (edit these if you prefer direct control)
    const IMAGE_OVERLAY_USE_CENTER = false; // set true to compute bounds from center + size
    const IMAGE_OVERLAY_BOUNDS = { north: 28.5483, south: 28.5283, east: -81.3692, west: -81.3892 };

    // Optional: when IMAGE_OVERLAY_USE_CENTER === true you can set these instead
    const IMAGE_OVERLAY_CENTER = { lat: 28.5383, lng: -81.3792 }; // center point for overlay
    const IMAGE_OVERLAY_WIDTH_METERS = 2000; // east-west width in meters
    const IMAGE_OVERLAY_HEIGHT_METERS = 2000; // north-south height in meters

    // --- Helper: validate bounds and compute from center+size when requested ---
    function isFiniteNumber(n) {
        return typeof n === 'number' && isFinite(n);
    }

    function validateBounds(bounds) {
        if (!bounds || typeof bounds !== 'object') return false;
        const { north, south, east, west } = bounds;
        if (![north, south, east, west].every(isFiniteNumber)) return false;
        // basic ordering checks
        if (!(north > south)) return false;
        // east/west can wrap the antimeridian in some cases; here we expect east > west for simplicity
        if (!(east > west)) return false;
        // lat range clamp
        if (north > 90 || south < -90) return false;
        if (east > 180 || west < -180) return false;
        return true;
    }

    // Approximate conversion from meters to degrees at a given latitude.
    // Uses mean earth radius approximation: 1 deg latitude ~= 111,320 meters.
    function metersToDegreesLat(meters) {
        return meters / 111320;
    }

    function metersToDegreesLng(meters, atLat) {
        // Longitude degrees scale with cos(latitude)
        const latRad = (atLat * Math.PI) / 180;
        const metersPerDeg = 111320 * Math.cos(latRad);
        // avoid divide-by-zero near poles
        return metersPerDeg === 0 ? 0 : meters / metersPerDeg;
    }

    function computeBoundsFromCenter(center, widthMeters, heightMeters) {
        if (!center || !isFiniteNumber(center.lat) || !isFiniteNumber(center.lng)) return null;
        const halfHeightDeg = metersToDegreesLat(heightMeters / 2);
        const halfWidthDeg = metersToDegreesLng(widthMeters / 2, center.lat);
        return {
            north: center.lat + halfHeightDeg,
            south: center.lat - halfHeightDeg,
            east: center.lng + halfWidthDeg,
            west: center.lng - halfWidthDeg
        };
    }

    function getOverlayBounds() {
        if (IMAGE_OVERLAY_USE_CENTER) {
            const b = computeBoundsFromCenter(IMAGE_OVERLAY_CENTER, IMAGE_OVERLAY_WIDTH_METERS, IMAGE_OVERLAY_HEIGHT_METERS);
            if (b && validateBounds(b)) return b;
            console.warn('Computed bounds from center are invalid, falling back to IMAGE_OVERLAY_BOUNDS if valid.');
        }
        if (validateBounds(IMAGE_OVERLAY_BOUNDS)) return IMAGE_OVERLAY_BOUNDS;
        // Last-resort: attempt to compute from center values if available
        const fallback = computeBoundsFromCenter(IMAGE_OVERLAY_CENTER, IMAGE_OVERLAY_WIDTH_METERS, IMAGE_OVERLAY_HEIGHT_METERS);
        if (fallback && validateBounds(fallback)) return fallback;
        // If nothing valid, return a very small default around center or zero-area box to avoid exceptions
        console.warn('No valid overlay bounds available — using tiny fallback box at (0,0).');
        return { north: 0.0001, south: -0.0001, east: 0.0001, west: -0.0001 };
    }

        function openTab(tabName) {
            // Deactivate all tab contents and navigation links
            console.log("(1)Switching a tab:");
            console.log("Tab Contents: ", tabContents);
            console.log("Nav Links: ", navLinks);

            tabContents.forEach(content => content.classList.remove('active'));
            navLinks.forEach(link => link.classList.remove('active'));
            console.log("(2)Removed All:");
            console.log("Tab Contents: ", tabContents);
            console.log("Nav Links: ", navLinks);

            // Activate the selected tab content and navigation link
            document.getElementById(tabName + '-content').classList.add('active');
            document.querySelector(`.nav-link[data-tab="${tabName}"]`).classList.add('active');
            console.log("(3)Activated Selected:");
            console.log("Tab Contents: ", tabContents);
            console.log("Nav Links: ", navLinks);

            // If switching to the 'home' tab, ensure the map initializes/resizes
            if (tabName === 'home') {
                // This calls the map initialization function defined later
                setTimeout(() => {
                    initMap();
                    // try to request user location when entering home (only once)
                    requestUserLocation().catch(() => {});
                }, 100);
            }
            // Focus chat inputs when opening their tabs and scroll to bottom
            if (tabName === 'civilian') {
                const input = document.getElementById('civilian-chat-input');
                const win = document.getElementById('civilian-chat-window');
                if (input) input.focus();
                if (win) win.scrollTop = win.scrollHeight;
            }
            if (tabName === 'enterprise') {
                const input = document.getElementById('enterprise-chat-input');
                const win = document.getElementById('enterprise-chat-window');
                if (input) input.focus();
                if (win) win.scrollTop = win.scrollHeight;
            }
        }

        // --- Google Maps Logic ---
        let map;
        let imageOverlay = null; // Variable to hold the GroundOverlay object

        function initMap() {
            // Only run if Google Maps API is loaded
            if (typeof google === 'undefined' || !google.maps) return;

            // Check if the map div exists and is visible
            const mapDiv = document.getElementById('map');
            if (!mapDiv) return;

            // Only create the map if it hasn't been created yet
            if (!map) {
                // Default location: Central Florida (using your existing values)
                //{ lat: 25.758925, lng: -80.376627}
                const initialPos = { lat: 30.758925, lng: -80.376627};

                map = new google.maps.Map(mapDiv, {
                    center: initialPos,
                    zoom: 10,
                    mapId: 'DEMO_MAP_ID', // Recommended for custom styling
                });

                // No default marker is placed here so the map starts clean.

                // ⭐ NEW: Load the image overlay immediately after map initialization
                // Mock coordinates for the image overlay (e.g., covering a 1-mile square)
                // Use configured overlay values (change at top of file)
                placeImageOverlay(IMAGE_OVERLAY_URL, getOverlayBounds());

                // After the map has been initialized, try requesting user location once
                requestUserLocation().catch(() => {});

            } else {
                // For existing map, this might trigger a resize if needed
                google.maps.event.trigger(map, 'resize');
            }
        }

        // ⭐ NEW FUNCTION: Fetches and places the image as a GroundOverlay
        function placeImageOverlay(imageUrl, bounds) {
            if (map && imageOverlay) {
                // Remove existing overlay before placing a new one (e.g., when updating feed)
                imageOverlay.setMap(null);
            }

            // Convert the plain bounds object into a google.maps.LatLngBounds object
            const latLngBounds = new google.maps.LatLngBounds(
                new google.maps.LatLng(bounds.south, bounds.west),
                new google.maps.LatLng(bounds.north, bounds.east)
            );

            // Create the GroundOverlay
            imageOverlay = new google.maps.GroundOverlay(
                imageUrl,
                latLngBounds,
                {
                    map: map, // Attaches the overlay to the map object
                    opacity: 0.8 // Optional: make it slightly transparent
                }
            );

            console.log(`GroundOverlay placed: ${imageUrl}`);
        }


        // You would typically use a function like this to dynamically update the image URL:
        
        function updateCameraFeedOverlay() {
            // Use configured overlay values (change IMAGE_OVERLAY_URL and IMAGE_OVERLAY_BOUNDS above)
            placeImageOverlay(IMAGE_OVERLAY_URL, getOverlayBounds());
        }
        

        // --- Chatbot Logic (Mock) for two separate chats ---
        function sendChatMessage(inputId, windowId) {
            const input = document.getElementById(inputId);
            const messageText = input.value.trim();
            const chatWindow = document.getElementById(windowId);

            if (messageText === "") return;

            const userMessageDiv = document.createElement('div');
            userMessageDiv.className = 'message-user';
            userMessageDiv.textContent = messageText;
            chatWindow.appendChild(userMessageDiv);
            input.value = '';

            setTimeout(() => {
                const botMessageDiv = document.createElement('div');
                botMessageDiv.className = 'message-bot';
                let response;
                response = `I received: "${messageText}"`;
                botMessageDiv.textContent = response;
                chatWindow.appendChild(botMessageDiv);
                chatWindow.scrollTop = chatWindow.scrollHeight;
            }, 400);

            chatWindow.scrollTop = chatWindow.scrollHeight;
        }

        // Wire civilian chat
        document.getElementById('civilian-send-btn').addEventListener('click', () => sendChatMessage('civilian-chat-input', 'civilian-chat-window'));
        document.getElementById('civilian-chat-input').addEventListener('keypress', function(e) { if (e.key === 'Enter') sendChatMessage('civilian-chat-input', 'civilian-chat-window'); });

        // Wire enterprise chat
        document.getElementById('enterprise-send-btn').addEventListener('click', () => sendChatMessage('enterprise-chat-input', 'enterprise-chat-window'));
        document.getElementById('enterprise-chat-input').addEventListener('keypress', function(e) { if (e.key === 'Enter') sendChatMessage('enterprise-chat-input', 'enterprise-chat-window'); });

        // --- Initialization ---
        document.addEventListener('DOMContentLoaded', () => {
            // wire locate button and coords display now that elements exist
            locateMeBtn = document.getElementById('locate-me-btn');
            coordsDisplay = document.getElementById('coords-display');
            if (locateMeBtn) locateMeBtn.addEventListener('click', () => requestUserLocation(true).catch(() => {}));

            // Attempt to request user location on initial load (will only prompt once)
            requestUserLocation().catch(() => {});

            // Add event listener for dynamic resizing (good practice)
            window.addEventListener('resize', () => {
                if (map) {
                    google.maps.event.trigger(map, 'resize');
                }
            });
        });

        // --- Chat Panel Toggle Logic ---
        // Chat panel/toggle removed; chats live in separate tabs

    // --- Geolocation (Use my location) ---
    let locateMeBtn = null;
    let coordsDisplay = null;
        let userLocationMarker = null;
        let userLocationRequested = false; // guard to avoid repeated prompts

        function showCoords(lat, lng) {
            coordsDisplay.textContent = `Lat: ${lat.toFixed(6)}, Lng: ${lng.toFixed(6)}`;
            coordsDisplay.style.display = 'block';
            setTimeout(() => { coordsDisplay.style.display = 'none'; }, 6000);
        }

        function requestUserLocation(force = false) {
            if (!navigator.geolocation) {
                console.warn('Geolocation is not supported by this browser.');
                return Promise.reject(new Error('Geolocation not supported'));
            }

            if (userLocationRequested && !force) {
                return Promise.resolve(null); // already asked once
            }

            userLocationRequested = true;

            // Optional UI feedback if button exists
            if (locateMeBtn) {
                locateMeBtn.disabled = true;
                locateMeBtn.textContent = 'Locating...';
            }

            return new Promise((resolve, reject) => {
                navigator.geolocation.getCurrentPosition(
                    (pos) => {
                        const lat = pos.coords.latitude;
                        const lng = pos.coords.longitude;

                        // Place or update marker only if the map is ready
                        if (typeof google !== 'undefined' && google.maps && typeof map !== 'undefined' && map) {
                            try {
                                if (userLocationMarker) userLocationMarker.setMap(null);
                                userLocationMarker = new google.maps.Marker({ position: { lat, lng }, map });
                                map.panTo({ lat, lng });
                                map.setZoom(15);
                            } catch (e) {
                                console.warn('Could not place marker (map not ready):', e);
                            }
                        }

                        showCoords(lat, lng);

                        if (locateMeBtn) {
                            locateMeBtn.disabled = false;
                            locateMeBtn.textContent = 'Use my location';
                        }

                        resolve({ lat, lng });
                    },
                    (err) => {
                        if (locateMeBtn) {
                            locateMeBtn.disabled = false;
                            locateMeBtn.textContent = 'Use my location';
                        }
                        console.warn('Geolocation error', err);
                        reject(err);
                    },
                    { enableHighAccuracy: true, timeout: 10000 }
                );
            });
        }

    // wiring of locate button is done after DOMContentLoaded when elements exist


    </script>

    <script>
    // --- Hurricanes CSV parsing and UI ---
    const STORMS_CSV_PATH = './maps_spaghetti_classic/storms_near_florida.csv';
    const SPAGHETTI_DIR = './maps_spaghetti_classic/';

    // Hold drawn hurricane overlays so we can clear them
    let hurricanePolylines = [];
    let hurricaneMarkers = [];

    // Simple CSV parser for the file format we have (sid,time_iso,time_ns,lat,lon)
    function parseStormsCsv(text) {
        const lines = text.trim().split(/\r?\n/).map(l => l.trim()).filter(Boolean);
        if (lines.length === 0) return {};
        const header = lines[0].split(',');
        const rows = lines.slice(1).map(line => {
            const cols = line.split(',');
            return {
                sid: cols[0].replace(/^b'|\'$/g, ''),
                time_iso: cols[1],
                time_ns: cols[2],
                lat: parseFloat(cols[3]),
                lon: parseFloat(cols[4])
            };
        });
        // Group by sid
        const groups = {};
        rows.forEach(r => {
            if (!groups[r.sid]) groups[r.sid] = [];
            groups[r.sid].push(r);
        });
        // Sort each storm's entries by time
        Object.keys(groups).forEach(sid => groups[sid].sort((a,b) => a.time_iso.localeCompare(b.time_iso)));
        return groups;
    }

    function clearHurricaneDrawings() {
        hurricanePolylines.forEach(pl => pl.setMap && pl.setMap(null));
        hurricaneMarkers.forEach(m => m.setMap && m.setMap(null));
        hurricanePolylines = [];
        hurricaneMarkers = [];
        // If using Leaflet in spaghetti files, we won't reuse those; we're drawing on Google Maps here.
    }

    function drawStormTimestamp(sid, timeIso) {
        // Find matching entries for that sid/time (there may be duplicates per timestamp)
        fetch(STORMS_CSV_PATH).then(r => r.text()).then(text => {
            const groups = parseStormsCsv(text);
            const entries = (groups[sid] || []).filter(e => e.time_iso === timeIso);
            if (entries.length === 0) {
                // If exact timestamp not found, try to find the single entry with that time by trimming ms
                alert('No entries found for that timestamp');
                return;
            }

            clearHurricaneDrawings();

            // Draw a polyline connecting all points for the storm (full track)
            const allPoints = groups[sid].map(p => ({ lat: p.lat, lng: p.lon }));
            const path = new google.maps.Polyline({
                path: allPoints,
                geodesic: true,
                strokeColor: '#FF0000',
                strokeOpacity: 0.8,
                strokeWeight: 3,
                map: map
            });
            hurricanePolylines.push(path);

            // Place a marker for each entry in the selected timestamp (usually 1)
            entries.forEach(ent => {
                const marker = new google.maps.Marker({ position: { lat: ent.lat, lng: ent.lon }, map });
                hurricaneMarkers.push(marker);

                // Try to find an image for this storm (a spaghetti HTML exists per sid). We'll embed the spaghetti HTML via iframe if present.
                const iframeSrc = SPAGHETTI_DIR + "b'" + sid + "'_spaghetti.html";
                const infoContent = document.createElement('div');
                infoContent.style.maxWidth = '320px';
                const title = document.createElement('div');
                title.textContent = `${sid} @ ${ent.time_iso}`;
                title.style.fontWeight = '600';
                title.style.marginBottom = '6px';
                infoContent.appendChild(title);

                // Add a small lat/lon line
                const coordLine = document.createElement('div');
                coordLine.textContent = `Lat: ${ent.lat.toFixed(4)}, Lng: ${ent.lon.toFixed(4)}`;
                coordLine.style.fontSize = '12px';
                coordLine.style.marginBottom = '6px';
                infoContent.appendChild(coordLine);

                // If the spaghetti HTML exists, embed it in an iframe (same-origin) else show a placeholder
                fetch(iframeSrc, { method: 'HEAD' }).then(() => {
                    const iframe = document.createElement('iframe');
                    iframe.src = iframeSrc;
                    iframe.width = '300';
                    iframe.height = '200';
                    iframe.style.border = '1px solid #ddd';
                    infoContent.appendChild(iframe);
                    const infowindow = new google.maps.InfoWindow({ content: infoContent });
                    marker.addListener('click', () => infowindow.open(map, marker));
                    // open immediately
                    infowindow.open(map, marker);
                }).catch(() => {
                    // HEAD failed — show a simple placeholder image if you have any images in the folder
                    const img = document.createElement('img');
                    img.src = SPAGHETTI_DIR + 'spaghetti_combined.html'; // fallback: user can edit to a png if available
                    img.alt = sid;
                    img.style.width = '100%';
                    img.style.height = 'auto';
                    infoContent.appendChild(img);
                    const infowindow = new google.maps.InfoWindow({ content: infoContent });
                    marker.addListener('click', () => infowindow.open(map, marker));
                    infowindow.open(map, marker);
                });
            });

            // Pan map to the selected point
            const first = entries[0];
            if (first && map) {
                map.panTo({ lat: first.lat, lng: first.lon });
                map.setZoom(6);
            }
        });
    }

    function buildHurricaneList() {
        const container = document.getElementById('hurricane-list');
        fetch(STORMS_CSV_PATH).then(r => r.text()).then(text => {
            const groups = parseStormsCsv(text);
            container.innerHTML = '';
            Object.keys(groups).sort().forEach(sid => {
                const stormDiv = document.createElement('div');
                stormDiv.className = 'storm-item';

                const header = document.createElement('div');
                header.className = 'storm-header';
                header.textContent = sid + `  (${groups[sid].length} points)`;
                stormDiv.appendChild(header);

                const tsList = document.createElement('div');
                tsList.className = 'storm-timestamps';
                groups[sid].forEach(entry => {
                    const ts = document.createElement('div');
                    ts.className = 'timestamp-item';
                    ts.textContent = entry.time_iso;
                    ts.addEventListener('click', () => {
                        // switch to home tab and draw
                        openTab('home');
                        drawStormTimestamp(sid, entry.time_iso);
                    });
                    tsList.appendChild(ts);
                });

                header.addEventListener('click', () => tsList.classList.toggle('active'));

                stormDiv.appendChild(tsList);
                container.appendChild(stormDiv);
            });
        }).catch(err => {
            container.innerHTML = '<p>Could not load storms CSV.</p>';
            console.error(err);
        });
    }

    // Build hurricane list when user opens Hurricanes tab (or on load)
    document.addEventListener('DOMContentLoaded', () => {
        // Build immediately so it's ready in the tab
        buildHurricaneList();
    });
    </script>

    <script
        src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBQQhjLjjYo4CCV3yil5JBCxpoBOz7qExY&callback=initMap"
        defer
    ></script>

</body>
</html>
    """)

    # Save the content to an HTML file
    file_name = "cvclone.html"
    try:
        # Write using UTF-8 to ensure Unicode characters in the HTML are handled
        with open(file_name, "w", encoding="utf-8") as f:
            f.write(html_content.strip())
        
        print(f"✅ Successfully generated '{file_name}'.")
        print("Open the file in your web browser to test the dynamic chat visibility.")

    except Exception as e:
        print(f"❌ An error occurred while writing the file: {e}")

# Run the function to generate the file
generate_cvclone_page_with_chat_logic()